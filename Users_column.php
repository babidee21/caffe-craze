...
.TableUser {
    padding: 20px;
    background-color: #141516; /* Dark background for the section */
    color: #fff; /* Set default text color to white */
}

.TableUser .customer-container {
    width: 100%;
    margin: auto;
}

.TableUser .customer-wrapper {
    overflow-x: auto; /* Ensures table responsiveness for smaller screens */
}
...
.TableUser table {
    ...
    border-radius: 8px;
    overflow: hidden; /* Ensures the table stays rounded */
    background-color: #2A2A2A; /* Dark background */
    color: #fff; /* Default table text color to white */
}
...
.TableUser td {
    padding: 12px 15px;
    text-align: left;
    color: #fff; /* Explicitly sets header and cell text to white */
    border: 1px solid #444; /* Optional: Add a subtle border for better visuals */
}
...
.TableUser tr:nth-child(odd) td {
    background-color: #555; 
}

.TableUser tr:nth-child(even) td {
...
<section class="title-banner text-center">
    <div class="container-fluid">
        <h1 class="color-white fw-400">Users Information</h1>
    </div>
</section>
    ...
    <div class="Usercontainer">
        ...
        <div class="User-container">
            <div class="User-wrapper">
                <table class="User table-striped">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>First Name</th>
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT id, FirstName, LastName, YourEmail, Role, Password, Status FROM signupusers";
                        $result = mysqli_query($conn, $sql);

                        if ($result) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                $status = $row['Status'];
                                $statusLabel = ($status == 'Active') ? 'Deactivate' : 'Activate';  // Toggle the action based on the status
                                echo "<tr>";
                                echo "<td>" . $row['id'] . "</td>";
                                echo "<td>" . $row['FirstName'] . "</td>";
                                echo "<td>" . $row['LastName'] . "</td>";
                                echo "<td>" . $row['YourEmail'] . "</td>";
                                echo "<td>" . $row['Role'] . "</td>";
                                echo "<td>" . $row['Password'] . "</td>";
                                echo "<td>" . $status . "</td>"; 
                                echo "<td><a href='update.php?id=" . $row['id'] . "' class='btn btn-" . ($status == 'Active' ? 'danger' : 'success') . "'>" . $statusLabel . "</a></td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='9'>Error fetching users: " . mysqli_error($conn) . "</td></tr>";
                        }
